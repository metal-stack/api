// Code generated by mockery v2.53.4. DO NOT EDIT.

package infrav2connect

import (
	context "context"

	connect "connectrpc.com/connect"

	infrav2 "github.com/metal-stack/api/go/metalstack/infra/v2"

	mock "github.com/stretchr/testify/mock"
)

// BMCServiceClient is an autogenerated mock type for the BMCServiceClient type
type BMCServiceClient struct {
	mock.Mock
}

// UpdateBMCInfo provides a mock function with given fields: _a0, _a1
func (_m *BMCServiceClient) UpdateBMCInfo(_a0 context.Context, _a1 *connect.Request[infrav2.UpdateBMCInfoRequest]) (*connect.Response[infrav2.UpdateBMCInfoResponse], error) {
	ret := _m.Called(_a0, _a1)

	if len(ret) == 0 {
		panic("no return value specified for UpdateBMCInfo")
	}

	var r0 *connect.Response[infrav2.UpdateBMCInfoResponse]
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *connect.Request[infrav2.UpdateBMCInfoRequest]) (*connect.Response[infrav2.UpdateBMCInfoResponse], error)); ok {
		return rf(_a0, _a1)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *connect.Request[infrav2.UpdateBMCInfoRequest]) *connect.Response[infrav2.UpdateBMCInfoResponse]); ok {
		r0 = rf(_a0, _a1)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*connect.Response[infrav2.UpdateBMCInfoResponse])
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, *connect.Request[infrav2.UpdateBMCInfoRequest]) error); ok {
		r1 = rf(_a0, _a1)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// NewBMCServiceClient creates a new instance of BMCServiceClient. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewBMCServiceClient(t interface {
	mock.TestingT
	Cleanup(func())
}) *BMCServiceClient {
	mock := &BMCServiceClient{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
